...
- Keep raw data unchanged in `raw_data/` and work on copies.
- Make sure reference FASTA and known-sites VCFs match the same build (e.g., hg38) and are indexed (fai, dict) where required.
- Use consistent sample identifiers (RG tags) when running AddOrReplaceReadGroups.
- For production workflows, use a workflow manager (Nextflow, Snakemake) and containers for reproducibility.
- VQSR requires cohort-scale data; for single-sample use hard filters or gather more samples.

---

## Do you want me to commit & push this file to the repository?
I can commit `read_gatk.md` and push it to your `variant_calling` repo now. Reply `yes` to proceed, or `no` if you prefer to review first.